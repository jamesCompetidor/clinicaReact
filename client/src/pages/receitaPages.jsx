...
const ReceitaPages = () => {
  ...
  const [atendimnentos, setAtendimentos] = useState([]);
        ...
        <Form>
          ...
          <Row className="mb-3">
            <Form.Group className="fs-5" as={Col} md="6">
              ...
              <Form.Select value={form.atendimento_id} onChange={(e) => setForm({...form, atendimento_id: e.target.value})} aria-label="Default select example">
                <option>Selecione o atendimento</option>
                {atendimnentos.map((atendimento) => (
            ...
            <Form.Group className="fs-5" as={Col} md="6">
              ...
              <Form.Control
                ...
                onChange={(e) =>
                  setForm({ ...form, data_receita: e.target.value })
                }
              />
            </Form.Group>
          </Row>
          ...
          <tbody>
            {receitas.map((receita) => (
              <tr key={receita.id}>
                <td>{receita.id}</td>
                <td>
                  {receita.medico_nome} - {receita.paciente_nome}
                </td>
                ...
                <td>
                  <button
                    ...
                    onClick={() => editar(receita)}
                  >
                    Editar
                  </button>
                  <button
                    className="btn btn-danger sm "
                    onClick={() => deletar(receita.id)}
                  >
                    Excluir
                  </button>
                </td>
